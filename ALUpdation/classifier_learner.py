...
import math
from .helper import classifier_predict_simple_uncertainty, rsquared
    ...
    def __init__(self, data_pack:DataSetup, fail_predictor=None, classifier_predict_uncertainty_func: Callable[[Any, np.ndarray], Tuple[np.ndarray, np.ndarray]]=classifier_predict_simple_uncertainty, redundancy_regressor=None, regressor_accuracy:Callable[[Any, np.ndarray, np.ndarray],float]=get_regressor_accuracy, get_valid_func:Callable[[np.ndarray],np.ndarray]=get_valid, X_train: np.ndarray=None, Y_train_success: np.ndarray=None, Y_train_perfs: np.ndarray=None, DESIGN_SPACE_DENSITY:int=100000, UNCERTAINTY_THRESHOLD:float=0.05, DROPOUT_RATE:float=0.2):
        ...
        if(X_train is None or Y_train_success is None or Y_train_perfs is None):
            ...
            for i in range(len(self.target_perfs)):
                ...
                self.target_perfs[i].estimator.fit(self._convert_to_one_hot(self.X_train), self.target_perfs[i].validity_checker(self.Y_train_perfs[:,i]))

    def query(self, batchNum:int, proximity_weight=0.5) -> np.ndarray:
        ...
        for perf in self.target_perfs:
            ...
            classifier_uncertainty = np.add(1-abs(2*preds-1), classifier_uncertainty)
        ...
        scores = proximity_weight * (1 - similarity_scores) + (1 - proximity_weight) * classifier_uncertainty
        ...
        with plt.style.context(plt.style.available[7]):
            ...
            plt.scatter(transformed_pool[:, 0], transformed_pool[:, 1], c=classifier_uncertainty, cmap='viridis', label='unlabeled')
